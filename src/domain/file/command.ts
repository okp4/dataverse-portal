 ...
 * or if multiple files in the StoreFileInput paylaod have the same id
 */
export type ResourceConflictError = ReturnType<typeof ResourceConflictError>

export const ResourceNotFoudError = (fileId: FileId) =>
  ({
...
/**
 *  Error for when the the id of a file to be removed is not found in the list of stored files in memory.
 */
export type ResourceNotFoudError = ReturnType<typeof ResourceNotFoudError>
...
export type Command = {
  ...
  removeFile: (fileId: FileId) => IOEither<ResourceNotFoudError, void>
}
